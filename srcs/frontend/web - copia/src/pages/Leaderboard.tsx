                  ...
                  <tbody className="divide-y divide-gray-700/50">
                    {getActiveLeaderboard().length > 0 ? (
                      getActiveLeaderboard().map((entry, index) => (
                        <tr key={`${entry.player_id}-${index}`} className="hover:bg-gray-700/30">
                          <td className="py-4 px-4">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${getMedalStyle(index)}`}>
                              {index + 1}
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-4 text-right">
                            <span className={`font-bold ${index < 3 ? 'text-blue-400' : 'text-gray-300'}`}>
                              {getDisplayValue(entry, index)}
                            </span>
                          </td>
                        </tr>
                      ))
                    ) : (
                      // If no data at all, show 10 placeholder rows
