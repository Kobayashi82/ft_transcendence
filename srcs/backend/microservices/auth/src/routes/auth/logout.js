...
const { logoutSchema } = require('../../schemas/login')

  async function logoutRoutes(fastify, options) {

    fastify.post('/logout', { schema: logoutSchema.schema }, async (request, reply) => {
      try {
        ...
        const refreshToken = request.cookies.refresh_token || (request.body && request.body.refresh_token);
