...
async function userRoutes(fastify, options) {
  ...
  // POST /users - Crear un nuevo usuario (público - registro)
  fastify.post('/', {
    ...
    try {
      ...
      // Solo admins pueden crear usuarios con rol de administrador
      if (
        userData.roles.includes('admin') && 
        (!request.user || !request.user.roles.includes('admin'))
      ) {
        reply.code(403).send({ error: 'No tienes permiso para crear usuarios administradores' })
        return
      }
