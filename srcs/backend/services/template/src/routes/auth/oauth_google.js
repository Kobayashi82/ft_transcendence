...
async function googleOauthRoutes(fastify, options) {
  ...
  fastify.get('/oauth/google/callback', { schema: oauthCallbackSchema }, async (request, reply) => {
    try {
      ...
      if (!storedProvider || storedProvider !== 'google') {
        fastify.logger.warn(`Invalid state in Google OAuth callback: ${state}`, {
          state,
          storedProvider,
          ip: request.ip
        })
      ...
      try {
        ...
        fastify.logger.error(`Google OAuth token exchange error: ${err.message}`, {
          error: err.message,
          stack: err.stack,
          ip: request.ip
        });
      ...
      try {
        ...
        fastify.logger.error(`Error fetching Google user info: ${err.message}`, {
          error: err.message,
          stack: err.stack,
          ip: request.ip
        });
      ...
      fastify.logger.info(`User data received from Google OAuth: ${userInfo.email}`, {
      ...
      if (!user) {
        ...
        if (existingUser) {
          ...
          fastify.logger.info(`Existing account linked with Google: ${existingUser.id}`, {
          ...
          fastify.logger.info(`New user created via Google OAuth: ${user.id}`, {
            userId: user.id,
            email: userInfo.email,
            googleId: userInfo.id
          })
      ...
      if (user.has_2fa) {
        ...
        fastify.logger.info(`2FA required for Google OAuth login: ${user.id}`, {
          userId: user.id,
          email: user.email
        })
