      ...
      try {
        ...
        try {
          ...
          fastify.logger.error(
            `42 OAuth token exchange error: ${err.message}`,
            {
              error: err.message,
              stack: err.stack,
              ip: request.ip,
        ...
        fastify.logger.info(
          `User data received from 42 OAuth: ${userInfo.email}`,
          {
        ...
        let user = await fastify.authDB.getUserByOAuthId(
        ...
        if (!user) {
          ...
          if (existingUser) {
            ...
            user = await fastify.authDB.updateUser(existingUser.id, {
              ...
              account_type:
                existingUser.account_type === "local" ? "local" : "42",
            });

            fastify.logger.info(
              `Existing account linked with 42: ${existingUser.id}`,
              {
