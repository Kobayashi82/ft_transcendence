...
async function securityPlugin(fastify, options) {
  ...
  const createHttpBreaker = (name) => {
    ...
    breaker.on("open", () => {
      fastify.logger.warn(
        `Circuit breaker '${name}' abierto - deteniendo peticiones`
      );
    });
    breaker.on("halfOpen", () => {
      fastify.logger.info(
        `Circuit breaker '${name}' en estado semi-abierto - probando recuperación`
      );
    });
    breaker.on("close", () => {
      fastify.logger.info(
        `Circuit breaker '${name}' cerrado - servicio recuperado`
      );
    });
  ...
  const encryptData = (text, key = fastify.config.encryption?.key) => {
    if (!key) {
      fastify.logger.warn(
        "Llave de cifrado no configurada, usando modo inseguro"
      );
      return text;
    }
    try {
  ...
  const lockAccount = async (email, durationSeconds = 300) => {
    // 5 minutos por defecto
    const key = `account:locked:${email}`;
    await fastify.redis.set(key, "locked", durationSeconds);
    // Registrar evento de seguridad
    if (fastify.metrics && fastify.metrics.auth) {
      fastify.metrics.auth.recordSecurityEvent("account_locked", "enforced");
    }
    fastify.logger.warn(`Cuenta bloqueada temporalmente: ${email}`, {
      email,
  ...
  const getFailedLoginAttempts = async (email) => {
    ...
    return parseInt((await fastify.redis.get(key)) || "0");
  };
  const incrementFailedLoginAttempts = async (email) => {
    ...
    await fastify.redis.set(key, newAttempts.toString(), 1800);
    // Loggear el incremento
    fastify.logger.warn(
      `Intento de login fallido para ${email}. Intentos: ${newAttempts}`,
      {
        email,
    ...
    if (newAttempts >= 5) {
      ...
      // Registrar evento de seguridad
      if (fastify.metrics && fastify.metrics.auth) {
        fastify.metrics.auth.recordSecurityEvent(
          "brute_force_attempt",
          "blocked"
        );
      }
  ...
  const resetFailedLoginAttempts = async (email) => {
    const key = `account:failed_attempts:${email}`;
    await fastify.redis.del(key);
    fastify.logger.info(`Reseteo de intentos fallidos para ${email}`, {
      email,
    });
    return true;
  };
...
module.exports = fp(securityPlugin, { name: "security" });
