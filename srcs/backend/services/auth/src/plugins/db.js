...
async function dbPlugin(fastify, options) {
  ...
  fastify.decorate("authDB", {
    ...
    async createUser(userData) {
      ...
      // Sanitizar datos
      const sanitizedUsername = fastify.security
        ? fastify.security.sanitizeInput(username)
        : username;
      ...
      try {
        return await fastify.db.transaction(async () => {
          // Insertar usuario
          let sql =
            "INSERT INTO users (email, username, password_hash, account_type, oauth_id";
          let params = [
            email,
            sanitizedUsername,
            password_hash,
          ...
          fastify.logger.info(`Usuario creado: ID=${userId}, Email=${email}`, {
            userId,
            email,
          });
    ...
    async updateUserSecure(id, userData) {
      const { email, username, password_hash, is_active, has_2fa, phone } =
        userData;
      ...
      try {
        ...
        fastify.logger.info(`Usuario actualizado de forma segura: ID=${id}`, {
          userId: id,
        });
        return await this.getUserById(id);
      } catch (err) {
    ...
    async getUserWithDecryptedData(id) {
      ...
      if (user.phone && fastify.security) {
        try {
          ...
          fastify.logger.error(`Error al descifrar teléfono: ${err.message}`, {
            userId: id,
          });
          user.phone = null;
        }
    ...
    async updateLastLogin(id) {
      try {
        ...
        fastify.logger.debug(`Último login actualizado: ID=${id}`, {
          userId: id,
        });
        return true;
      } catch (err) {
        fastify.logger.error(
          `Error al actualizar último login: ${err.message}`,
          { userId: id }
        );
    ...
    async deleteUser(id) {
      try {
        ...
        fastify.logger.info(`Usuario eliminado (soft delete): ID=${id}`, {
          userId: id,
        });

        return { id, deleted: true };
      } catch (err) {
        fastify.logger.error(`Error al eliminar usuario: ${err.message}`, {
          userId: id,
        });
    ...
    async updateUserRoles(userId, roles) {
      try {
        return await fastify.db.transaction(async () => {
          ...
          fastify.logger.info(`Roles actualizados para usuario: ID=${userId}`, {
            userId,
            roles,
          });
        ...
        fastify.logger.error(
          `Error al actualizar roles de usuario: ${err.message}`,
          { userId }
        );
    ...
    async createRefreshToken(userId, token, expiresIn) {
      ...
      try {
        ...
        fastify.logger.debug(
          `Token de refresco creado para usuario: ID=${userId}`,
          {
            userId,
            tokenId: result.id,
            expiresAt,
        ...
        fastify.logger.error(
          `Error al crear token de refresco: ${err.message}`,
          { userId }
        );
    ...
    async getRefreshToken(token) {
      try {
        ...
        fastify.logger.error(
          `Error al obtener token de refresco: ${err.message}`
        );
        throw err;
      }
    },
    ...
    async revokeRefreshToken(token) {
      try {
        ...
        fastify.logger.debug(
          `Token de refresco revocado: ${token.substring(0, 8)}...`
        );

        return true;
      } catch (err) {
        fastify.logger.error(
          `Error al revocar token de refresco: ${err.message}`
        );
    ...
    async revokeAllRefreshTokens(userId) {
      try {
        ...
        fastify.logger.info(
          `Todos los tokens de refresco revocados para usuario: ID=${userId}`,
          {
            userId,
        ...
        fastify.logger.error(
          `Error al revocar todos los tokens de refresco: ${err.message}`,
          { userId }
        );
        throw new Error(
          `Error al revocar todos los tokens de refresco: ${err.message}`
      ...
      try {
        ...
        if (phone && fastify.security) {
          try {
            ...
            fastify.logger.error(
              `Error al cifrar teléfono para 2FA: ${err.message}`,
              { userId }
            );
          }
        }
        ...
        if (existing) {
          ...
          fastify.logger.info(
            `Configuración 2FA actualizada para usuario: ID=${userId}, Tipo=${type}`,
            {
              userId,
              type,
            }
          ...
          fastify.logger.info(
            `Configuración 2FA creada para usuario: ID=${userId}, Tipo=${type}`,
            {
              userId,
              type,
            }
        ...
        fastify.logger.error(`Error al gestionar 2FA: ${err.message}`, {
          userId,
          type,
        });
      ...
      try {
        ...
        if (config && config.phone && fastify.security) {
          try {
            ...
            fastify.logger.error(
              `Error al descifrar teléfono para 2FA: ${err.message}`,
              { userId }
            );
            config.phone = null;
          }
        ...
        fastify.logger.error(
          `Error al obtener configuración 2FA: ${err.message}`,
          { userId, type }
        );
        throw err;
      }
    ...
    async verify2FA(userId, type) {
      try {
        ...
        fastify.logger.info(
          `2FA verificado para usuario: ID=${userId}, Tipo=${type}`,
          {
            userId,
            type,
          }
        ...
        fastify.logger.error(`Error al verificar 2FA: ${err.message}`, {
          userId,
          type,
        });
    ...
    async enable2FA(userId, type) {
      try {
        ...
        fastify.logger.info(
          `2FA habilitado para usuario: ID=${userId}, Tipo=${type}`,
          {
            userId,
            type,
          }
        ...
        fastify.logger.error(`Error al habilitar 2FA: ${err.message}`, {
          userId,
          type,
        });
    ...
    async disable2FA(userId) {
      try {
        ...
        fastify.logger.info(`2FA deshabilitado para usuario: ID=${userId}`, {
          userId,
        });

        return true;
      } catch (err) {
        fastify.logger.error(`Error al deshabilitar 2FA: ${err.message}`, {
          userId,
        });
    ...
    async createPasswordResetToken(userId, token, expiresIn = 3600) {
      // 1 hora por defecto
      const expiresAt = new Date(Date.now() + expiresIn * 1000).toISOString();
      ...
      try {
        ...
        fastify.logger.info(
          `Token de reseteo de contraseña creado para usuario: ID=${userId}`,
          {
            userId,
            expiresAt,
          }
        ...
        fastify.logger.error(
          `Error al crear token de reseteo: ${err.message}`,
          { userId }
        );
    ...
    async getPasswordResetToken(token) {
      try {
        ...
        fastify.logger.error(
          `Error al obtener token de reseteo: ${err.message}`
        );
        throw err;
      }
    },
    ...
    async usePasswordResetToken(token) {
      try {
        ...
        fastify.logger.info(
          `Token de reseteo de contraseña utilizado: ${token.substring(
            0,
            8
          )}...`
        );
    ...
    async createSession(userId, tokenId, ip, userAgent, expiresIn) {
      ...
      try {
        ...
        fastify.logger.error(`Error al crear sesión: ${err.message}`, {
          userId,
          ip,
        });
    ...
    async getActiveSessions(userId) {
      try {
        ...
        fastify.logger.error(
          `Error al obtener sesiones activas: ${err.message}`,
          { userId }
        );
        throw err;
      }
    ...
    async revokeSession(id, userId) {
      try {
        ...
        fastify.logger.info(
          `Sesión revocada: ID=${id} para usuario ID=${userId}`,
          {
            sessionId: id,
            userId,
          }
        ...
        fastify.logger.error(`Error al revocar sesión: ${err.message}`, {
          sessionId: id,
          userId,
        });
    ...
    async revokeAllSessions(userId, currentTokenId = null) {
      try {
        ...
        const result = await fastify.db.update(sql, params, "sessions");
        ...
        fastify.logger.error(`Error al revocar sesiones: ${err.message}`, {
          userId,
        });
    ...
    async getUsersCount() {
      try {
        ...
        fastify.logger.error(
          `Error al obtener conteo de usuarios: ${err.message}`
        );
        return 0;
      }
    },
    ...
    async getActiveUsersCount() {
      try {
        ...
        fastify.logger.error(
          `Error al obtener conteo de usuarios activos: ${err.message}`
        );
        return 0;
      }
    },
    ...
    async getSessionsCount() {
      try {
        ...
        fastify.logger.error(
          `Error al obtener conteo de sesiones: ${err.message}`
        );
        return 0;
      }
    },
    ...
    async getUserDevices(userId) {
      ...
      try {
        ...
        fastify.logger.error(
          `Error al obtener dispositivos del usuario: ${err.message}`,
          { userId }
        );
