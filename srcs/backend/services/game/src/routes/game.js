...
async function routes(fastify) {
  ...
  fastify.post('/create', { schema: schemas.createGame }, async (request, reply) => {
    ...
    if (message) { reply.code(409); return { success: false, message: message }}
  ...
  fastify.post('/:gameId/start', { schema: schemas.gameAction }, async (request, reply) => {
    ...
    if (!gameManager.getGame(gameId)) { reply.code(404); return { success: false, message: 'Game not found' }}
  ...
  fastify.post('/:gameId/pause', { schema: schemas.gameAction }, async (request, reply) => {
    ...
    if (!game) { reply.code(404); return { success: false, message: 'Game not found' }}
  ...
  fastify.post('/:gameId/move', async (request, reply) => {
    ...
    if (success) { return { success: true, message: 'Movement processed successfully' }}
    else { reply.code(404); return { success: false, message: 'Game not found' }}
  });
  ...
  fastify.post('/:gameId/position', async (request, reply) => {
    ...
    if (success) { return { success: true, mesaddPlayeraddPlayersage: 'Position updated successfully' }}
    else { reply.code(404); return { success: false, message: 'Game not found' }}
  });
  ...
  fastify.get('/:gameId', async (request, reply) => {
    ...
    if (gameState) { return { success: true, gameState }}
    else { reply.code(404); return { success: false, message: 'Game not found' }}
  });
