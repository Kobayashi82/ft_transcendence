  ...
  for (const routeConfig of Object.values(routes)) {
    if (typeof routeConfig === 'object' && routeConfig.method) {
      const methodArray = Array.isArray(routeConfig.method) ?
        routeConfig.method : [routeConfig.method]      
...
async function proxyPlugin(fastify, options) {
  ...
  fastify.addHook('preHandler', async (request, reply) => {
      ...
      if (typeof routeConfig === 'object') {
        ...
        if (routeConfig.method) {
          allowedMethods = Array.isArray(routeConfig.method) ?
            routeConfig.method.map(m => m.toUpperCase()) : [routeConfig.method.toUpperCase()]
        }
      }
    }
    ...
    if (allowedMethods && !allowedMethods.includes(method)) {
      ...
      if (fastify.metrics && fastify.metrics.proxy) {
        fastify.metrics.proxy.recordMethodNotAllowed(routePath, method, allowedMethods)
      }
    ...
    if (!hasRequiredRoles(userRoles, requiredRoles)) {
      ...
      if (fastify.metrics && fastify.metrics.security) {
        fastify.metrics.security.recordRoleCheck(routePath, 'denied')
        fastify.metrics.security.recordUnauthorizedAccess(routePath, request.ip)
      }
    ...
    if (fastify.metrics && fastify.metrics.security) {
      fastify.metrics.security.recordRoleCheck(routePath, 'allowed')
    }
  })
  ...
  fastify.setErrorHandler((error, request, reply) => {
    ...
    if (error.code === 'ETIMEDOUT') {
      ...
      if (fastify.metrics && fastify.metrics.proxy) {
        fastify.metrics.proxy.recordConnectionError(
          request.headers['x-target'],
          request.method,
          'TIMEOUT'
        )
      }
...
module.exports = fp(proxyPlugin, { name: 'proxy', dependencies: ['metrics', 'logger', 'auth'] })