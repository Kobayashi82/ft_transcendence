    ...
    onExceeded: (req) => {
      for (const route of routeRateLimits) {
        if (
          ...
          fastify.logger.warn(
            `Rate-Limit: Request from ${req.ip} to ${req.method} ${req.url} blocked. Limit of ${route.limit.max} requests in ${route.limit.timeWindow} seconds exceeded.`
          );
          return;
        }
      }
      ...
      fastify.logger.warn(
        `Rate-Limit: Request from ${req.ip} to ${req.method} ${req.url} blocked. Limit of 100 requests in 60 seconds exceeded.`
      );
      return;
    },
...
module.exports = fp(rateLimitPlugin, {
  name: "rate-limit",
  dependencies: ["@fastify/redis", "logger"],
});
