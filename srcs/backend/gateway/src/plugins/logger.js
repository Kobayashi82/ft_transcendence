...
async function loggerPlugin(fastify, options) {
  ...
  const logger = {
    ...
    logstash: {
      info: (data, msg) => {
        if (typeof data === 'object' && data.message && !msg) sendToLogstash('info', data.message, data)
        else                                                  sendToLogstash('info', msg || 'Info log', data)
      },
      error: (data, msg) => {
        if (typeof data === 'object' && data.message && !msg) sendToLogstash('error', data.message, data)
        else                                                  sendToLogstash('error', msg || 'Error log', data)
      },
      warn: (data, msg) => {
        if (typeof data === 'object' && data.message && !msg) sendToLogstash('warn', data.message, data)
        else                                                  sendToLogstash('warn', msg || 'Warning log', data)
      },
      debug: (data, msg) => {
        if (typeof data === 'object' && data.message && !msg) sendToLogstash('debug', data.message, data)
  ...
  fastify.post('/internal/logs', async (request, reply) => {
    ...
    if (fastify.metrics && fastify.metrics.logging) {
      fastify.metrics.logging.recordLogsProcessed(service || 'unknown', level || 'info')
    }
...
module.exports = fp(loggerPlugin, { name: 'logger', dependencies: ['metrics'] })
